...
class ProgressBar(object):
    def __init__(
        self, minValue=0, maxValue=0, width=None, total_items=None, fd=sys.stderr
    ):
    ...
    def display(self):
        sys.stdout.write(
            "\r%3s / %3s items: %s\r" % (self.complete, self.items, str(self))
        )
    ...
    def __str__(self):
        ...
        return (
            "["
            + "#" * widthFilled
            + " " * (self.width - widthFilled)
            + "]"
            + " %5.1f%% of %s"
            % (percentFilled * 100.0, self.__numStr__(self.max / 1024))
        )
    ...
    def compress_the_file(self, zip_file_name, files_to_compress):
        ...
        if fileOpened:  # Supported from Python 2.5 ??
            ...
            try:
                filename.write(
                    files_to_compress,
                    os.path.basename(files_to_compress),
                ...
                if e.errno == errno.ENOENT:
                    ...
                    raise AptOfflineErrors(
                        "Ignoring err ENOENT, multiarch package name clash %s\n"
                        % (files_to_compress)
                    )
...
class FileMgmt(object):
    ...
    def move_file(self, src, dest):
        ...
        try:
            ...
            raise AptOfflineLibShutilError(
    ...
    def find_dup(self, dir):
                ...
                if file == yfile:
                    if not (xpath + "/" + yfile == path + "/" + file):
                        if [
                            xpath + "/" + yfile,
                            path + "/" + file,
                        ] in self.duplicate_files:
                            break
                    else:
                        self.duplicate_files += [
                            [xpath + "/" + yfile, path + "/" + file]
                        ]
        ...
        for x, y in self.duplicate_files:
            while number < len - 1:
                if (
                    x in self.duplicate_files[number]
                    or y in self.duplicate_files[number]
                ):
                    num += 1
                    print(num)
                    if num > 1:
...
class MyThread(threading.Thread):
    ...
    def __init__(
        self, WorkerFunction, requestQueue=None, responseQueue=None, NumOfThreads=1
    ):
        ...
        self.thread_pool = [
            threading.Thread(target=self.run, args=()) for i in range(self.threads)
        ]
